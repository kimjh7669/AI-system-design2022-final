...
def bbox_transform_3d(ex_rois_2d, ex_rois_3d, gt_rois):
    ...
    try:
        ex_rois_3d = ex_rois_3d.cpu().detach().numpy()
    except:
        pass
...
def im_detect_3d(im, net, rpn_conf, preprocess, p2, gpu=0, synced=False):
    ...
    # im = torch.from_numpy(im[np.newaxis, :, :, :])
    ...
    # src_3d = torch.from_numpy(rpn_conf.anchors[tracker, 4:]).cpu().type(torch.FloatTensor)
...
def test_kitti_3d_q(dataset_test, net, rpn_conf, results_path, test_path, results_save_path, use_log=True):
    """
    Test the KITTI framework for object detection in 3D
    """
    ...
    backbone_time = 0
    head_time = 0
    for i in range(len(net.s_time_list)):
...
def test_kitti_3d_q_for_plot(dataset_test, net, rpn_conf, results_path, test_path, results_save_path, use_log=True):
    """
    Test the KITTI framework for object detection in 3D
    """
    ...
    try:
        ...
        name = "result"
    file = open(os.path.join(results_save_path, name + '_for_plot' + '.txt'), 'w')
...
def test_kitti_3d_q_for_inference_time(dataset_test, net, rpn_conf, results_path, test_path, results_save_path, use_log=True):
    """
    Test the KITTI framework for object detection in 3D
    """
