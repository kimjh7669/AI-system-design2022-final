...
class RPN(nn.Module):
    ...
    def forward(self, x):
        starter, ender = torch.cpu.Event(enable_timing=True), torch.cpu.Event(enable_timing=True)
        s_time = time()
        x.detach()
        batch_size = x.size(0)
...
def build(conf, phase='train'):
    ...
    src_weights = torch.load('/media/hdd/jhkim/git/M3D-RPN/output/kitti_3d_multi_main_resnet50/weights/model_100000_pkl', map_location=torch.device('cpu'))
    ...
    modules_to_replace = quan.find_modules_to_quantize_head(rpn_net, args_q.quan, logger=quan_logger)
    rpn_net = quan.replace_module_by_names_head(rpn_net, modules_to_replace)
